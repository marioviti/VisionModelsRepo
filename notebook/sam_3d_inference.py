...
class Inference:
    ...
    def __init__(self, config_file: str, compile: bool = False):
        ...
        # Patch the pipeline to be memory efficient
        self._pipeline.__class__ = MemoryEfficientInferencePipeline
...
class MemoryEfficientInferencePipeline(InferencePipelinePointMap):
    def run(
        self,
        ...
        def to_cpu(module):
            ...
            if hasattr(module, "to"):
                ...
                # Handle wrappers like MoGe/DepthModel
                module.device = torch.device("cpu")
                module.model.to("cpu")
                
            torch.cuda.empty_cache()
        
        def to_device(module):
            if module is None:
                return
            if hasattr(module, "to"):
        ...
        # Ensure everything is on CPU initially
        if hasattr(self, "depth_model"):
            to_cpu(self.depth_model)
        for model in self.models.values():
            to_cpu(model)
        for embedder in self.condition_embedders.values():
            to_cpu(embedder)
        ...
        # 1. Depth Estimation
        if hasattr(self, "depth_model"):
            to_device(self.depth_model)

        with self.device: 
            pointmap_dict = self.compute_pointmap(image, pointmap)

        if hasattr(self, "depth_model"):
            to_cpu(self.depth_model)  # Offload depth model
        ...
        with self.device:
            ...
            to_device(self.models["ss_generator"])
            to_device(self.models["ss_decoder"])
            to_device(self.pose_decoder)
            # Conditions embedders
            if "ss_condition_embedder" in self.condition_embedders:
                 to_device(self.condition_embedders["ss_condition_embedder"])
            ...
            # Offload Stage 1 models
            to_cpu(self.models["ss_generator"])
            to_cpu(self.models["ss_decoder"])
            to_cpu(self.pose_decoder)
            if "ss_condition_embedder" in self.condition_embedders:
                 to_cpu(self.condition_embedders["ss_condition_embedder"])
            ...
            # 3. Stage 2: Sparse Latent
            to_device(self.models["slat_generator"])
            if "slat_condition_embedder" in self.condition_embedders:
                 to_device(self.condition_embedders["slat_condition_embedder"])
            ...
            # Offload Stage 2 models
            to_cpu(self.models["slat_generator"])
            if "slat_condition_embedder" in self.condition_embedders:
                 to_cpu(self.condition_embedders["slat_condition_embedder"])
            
            # 4. Decoding
            # slat_decoder_mesh, slat_decoder_gs, etc.
            # We need to load specific decoders based on formats
            formats = self.decode_formats if decode_formats is None else decode_formats
            if "mesh" in formats:
                to_device(self.models["slat_decoder_mesh"])
            if "gaussian" in formats:
                to_device(self.models["slat_decoder_gs"])
            if "gaussian_4" in formats:
                to_device(self.models["slat_decoder_gs_4"])
            ...
            # Offload decoders
            if "mesh" in formats:
                to_cpu(self.models["slat_decoder_mesh"])
            if "gaussian" in formats:
                to_cpu(self.models["slat_decoder_gs"])
            if "gaussian_4" in formats:
                to_cpu(self.models["slat_decoder_gs_4"])
